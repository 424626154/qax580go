...
type WxFeedbackController struct {
	beego.Controller
}

func (c *WxFeedbackController) Get() {
...
func getFeedbackAccessToken(code string, c *WxFeedbackController) {
	...
	if err := json.Unmarshal(body, &atj); err == nil {
		...
		beego.Debug(atj)
		if atj.ErrCode == 0 {
			getFeedbackUserInfo(atj.AccessToken, atj.OpenID, c)
		}
	} else {
...
func getFeedbackUserInfo(access_toke, openid string, c *WxFeedbackController) {
	isdebug := "true"
	iniconf, err := config.NewConfig("json", "conf/myconfig.json")
	if err != nil {
		beego.Debug(err)
