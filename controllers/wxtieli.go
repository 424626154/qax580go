...
const (
	tl_token          = "tieli580"
	tl580_name        = "gh_8069470cf03a"
	tl_error_info     = "我们正在努力成为一个有情怀的免费信息网站"
	tl_null_info0     = "未搜索到相关信息"
	tl_null_info1     = "未搜索到有关&的信息"
	tl_subscribe_info = "欢迎关注铁力广告网，我们正在努力成为一个有情怀的免费信息发布平台，为大家服务"
	tl_about_info     = "【客服服务】\n铁力广告网为您服务，您的疑问可以直接在回复中告诉我们，我们会及时为您解答"
	tl_content_url    = "http://www.baoguangguang.cn/content?op=con&id=s%"
...
// 	MsgType      CDATAText
// }
type WXTlController struct {
	beego.Controller
}

func (c *WXTlController) Get() {
	verification_tl(c)
}
func (c *WXTlController) Post() {
	responseMsg_tl(c)
}

//验证签名
func verification_tl(c *WXTlController) {
	beego.Debug("valid ")
	...
	tmpArr := []string{tl_token, timestamp, nonce}
	sort.Strings(tmpArr)
	tmpStr := ""
...
//响应消息
func responseMsg_tl(c *WXTlController) {
	...
	if err != nil {
		...
		response_xml = responseTypeMsgTl(body, requestType.MsgType)
	}
...
//根据类型解析消息
func responseTypeMsgTl(body []byte, msgType string) string {
	response_xml := ""
	switch msgType {
	//文字
	case "text":
		...
		if err != nil {
			beego.Debug(err.Error())
			response_xml = responseTextMsg(requestBody.FromUserName, tl_error_info, tl580_name)
		} else {
			requestBody := &TextRequestBody{}
			err := xml.Unmarshal(body, requestBody)
			if err != nil {
				...
				response_xml = responseTextMsg(requestBody.FromUserName, tl_jieshao_info, tl580_name)
			} else if strings.Index(requestBody.Content, "发布") >= 0 {
				response_xml = responseTextMsg(requestBody.FromUserName, tl_function_info, tl580_name)
			} else {
				//是否存在关键字
				key_count := int32(0)
				beego.Debug("GetKeywordsCount :", requestBody.Content)
				...
				if key_count > 0 {
					...
					if err != nil {
						...
						if len(objs) > 0 {
							response_xml = responseKeyXML(requestBody.FromUserName, requestBody.Content, objs, tl580_name)
						} else {
					...
					if len(posts) > 0 {
						response_xml = responseImageTextXML(requestBody.FromUserName, requestBody.Content, posts, tl580_name)
					} else {
						response_xml = responseCustomerService(requestBody.FromUserName, requestBody.ToUserName)
					}
				}
	...
	case "voice":
		...
		if err != nil {
			...
			response_xml = responseImageTextXML(requestBody.FromUserName, requestBody.Recognition, posts, tl580_name)
	...
	case "event":
		...
		if err != nil {
			...
			beego.Debug("tieli 580 Event:", requestBody.Event, "EventKey", requestBody.EventKey)
			//自定义点击事件
			if requestBody.Event == "CLICK" {
				...
				if requestBody.EventKey == "recommend" {
					...
					if len(posts) != 0 {
						response_xml = responseImageTextXML(requestBody.FromUserName, "", posts, tl580_name)
					} else {
						response_xml = responseTextMsg(requestBody.FromUserName, "今日已无更多推荐信息", tl580_name)
					}
					//关于
				} else if requestBody.EventKey == "about" {
					response_xml = responseAbout(requestBody.FromUserName, tl580_name, tl_about_info)
				} else if requestBody.EventKey == "today" { //今日580
					response_xml = responseToday(requestBody.FromUserName, tl580_name)
				} else {

				}
				//关注
			} else if requestBody.Event == "subscribe" {
				response_xml = responseTextMsg(requestBody.FromUserName, tl_subscribe_info, tl580_name)
				beego.Debug("subscribe tieli580")
				eventSubscribe(requestBody, tl_appid, tl_secret)
			} else {
				//其他类型
				response_xml = responseTextMsg(requestBody.FromUserName, tl_error_info, tl580_name)
			}
...
// 	null_info := tl_null_info0
// 	if len(content) > 0 {
// 		null_info = strings.Replace(tl_null_info1, "&", content, -1)
// 	}
// 	textResponseBody := &TextResponseBody{}
// 	textResponseBody.FromUserName = value2CDATA(tl580_name)
// 	textResponseBody.ToUserName = value2CDATA(toUserName)
// 	textResponseBody.MsgType = value2CDATA("text")
// 	textResponseBody.Content = value2CDATA(null_info)
// 	textResponseBody.CreateTime = time.Duration(time.Now().Unix())
// 	tl_error_info, _ := xml.MarshalIndent(textResponseBody, " ", "  ")
// 	return string(tl_error_info)
// }
...
// 	textResponseBody.ToUserName = value2CDATA(toUserName)
// 	textResponseBody.MsgType = value2CDATA("text")
// 	textResponseBody.Content = value2CDATA(tl_about_info)
// 	textResponseBody.CreateTime = time.Duration(time.Now().Unix())
// 	about_xml, _ := xml.MarshalIndent(textResponseBody, " ", "  ")
// 	return string(about_xml)
// }
...
// 			imageTextResponseItem.Url = value2CDATA(strings.Replace(tl_content_url, "s%", fmt.Sprintf("%d", posts[i].Id), -1))
